...
- 2026-09-15: the client row stopped being a stacked list. At the user's request ("too big, too long, and a bit ugly… just images with the name… span to the very left, to the very right… maybe on the mobile version there'll only be two images or cards") `provider-pane.tsx` was **deleted** and `use-now.tsx` now renders `ClientMarquee` — a full-bleed marquee of client marks + names (a CSS-keyframe loop then; **superseded 2026-09-16** by the JS-owned rAF travel + swipe — see `client-marquee.md`). The section lost its `px-6 md:px-12` (moved onto the `<h2>`) so the row reaches both viewport edges. `kind` / `access` / `alt` were dropped from `client-providers.ts`; the card itself is now the OpenRouter link. **Retired the row-scale lockstep with `model-pane.tsx`** (see `model-slideshow.md`). Measured live at 3000: marquee bottom → email field still **176px desktop / 166px at 375px** (`pb` untouched), `#use-now` bottom === `#contact` top, `scrollWidth === innerWidth` at both widths, 7 a11y links (duplicates `inert`). New skill: `frontend/client-marquee.md`; `provider-list.md` archived; `infinite-slider.md` updated with why the CSS marquee was preferred over `InfiniteSlider`.
