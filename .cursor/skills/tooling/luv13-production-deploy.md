...
- 2026-09-16 (night, Kor only — no GitHub, per user): deploy for the model-ID copy control. New `components/models/model-id-copy.tsx` (`ModelIdCopy`, uses shared `copyText` from `lib/clipboard.ts`) wired into `marketing/model-pane.tsx`; plus `model-id-copy.test.tsx`, `lib/model-ids.test.ts`, `lib/models.ts` / `model-directory.ts` / `model-slides.ts` / `client-marquee.tsx` / `contact-16.tsx` / `image-zoom.tsx` / `globals.css`. Find-diff 134/134 — clean, so **no** `rm` needed. `luv13-web` recreated 20:49 local. All routes 200 (`/keys`, `/top-up` 307 by design); vhost 200; stale assets still 404; new rasters 200. 8/8 vitest pass. Verified live: all seven `luv13/*` IDs expose `Copy <id> to clipboard`, click flips aria-label→`Model ID copied` and SR status announces it. Contact e-mail is now `hi@luv13.ai` (was `.com`). API/proxy were recreated by the user at 20:25/20:26 — left untouched.

### Model ID copy: where it lives (avoid false alarms)
`ModelIdCopy` is used **only** in the home `#models` pane rows (`marketing/model-pane.tsx`). The `/models` directory page prints the same `luv13/*` IDs but has **no** copy buttons — that is intended, not a broken deploy. Verify the feature on `/` (7 controls), not `/models`.
Grep for `Copy .* to clipboard` (the `aria-label`), not `Copy model ID` — the button text *is* the ID, so there is no such literal string.

- 2026-09-16 (night, 2, Kor only — no GitHub, per user): deploy for the iOS touch icon. New `web/app/apple-icon.png` (180², cut from `BRAND_ASSETS/LUV13.png`, flattened onto `#ffffff` — iOS composites a transparent touch icon on black) plus `metadata.appleWebApp = { capable: false, title: "luv13" }` in `app/layout.tsx`. Find-diff **135 local / 134 remote** — the only delta was the new file, so no `rm`. Tar-sync → `luv13-web` recreated **03:58Z**; `/api/health` 200; head carries `<title>luv13</title>` + `apple-mobile-web-app-title` + `apple-touch-icon 180x180` and **no** `mobile-web-app-capable`; vhost `:80` 200 for `/` and `/apple-icon.png`; API/proxy Created unchanged (`03:26`/`03:25`). Public `https://luv13.ai/apple-icon.png?apple-icon.0d928l521dp0g.png` → 200, `md5 cfdf47a6610eb3f656e13e725e85f6f9` = local, `cf-cache-status: MISS`. **File-convention icons are content-hashed by Next** (`?apple-icon.<hash>.png`), so unlike the `BRAND_ASSETS` rasters a re-cut icon is never a stale-URL problem — do **not** reach for `RASTER_VERSION` here. Skill: `frontend/site-icons-and-metadata.md`.

### iOS tiles: the favicon is not the avatar (avoid a false alarm)
`/icon.png` being the correct brand mark does **not** mean the Safari start-page tile shows it. With no `<link rel="apple-touch-icon">` iOS falls back to a **crop of the rendered page** — the LUV13 tile showed the model-slideshow covers for months for exactly that reason. The tile **label** is the page `<title>` (~20 chars, truncated) cached **on the device**, so a title fixed in code can still read the old string on a phone that visited before the fix; have the user long-press → Delete the suggestion and revisit. Verify both levers over `curl`, not by looking at the phone: the head must carry `<link rel="apple-touch-icon" ... sizes="180x180">` and must **not** carry `mobile-web-app-capable` (Next's `appleWebApp.capable` defaults to `true`).
